...
function DashboardPage() {
  ...
  const getColorClasses = (color: string) => {
    const colorMap: { [key: string]: { border: string; shadow: string; bg: string; text: string; gradient: string } } = {
      blue: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {stats && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-10">
              <StatCard label="Total" value={stats.totalTasks} color="blue" />
              <StatCard label="Completed" value={stats.completedTasks} color="emerald" />
              <StatCard label="Pending" value={stats.pendingTasks} color="yellow" />
              <StatCard label="High Priority" value={stats.highPriorityTasks} color="red" />
              <StatCard label="Overdue" value={stats.overdueTasks} color="orange" />
              <StatCard label="Today" value={stats.todayTasks} color="purple" />
            </div>
          )}
          ...
          <div className="animate-slideInRight" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl md:text-3xl font-bold text-white">Your Tasks</h2>
              {tasks.length > 0 && (
